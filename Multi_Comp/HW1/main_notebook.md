...
According to what we proved in the previous section, when \(H_0\) is
correct, the distribution function of P-Value is independent of the
parameters: \(n\),\(sd\). Therefore the distrubtion function stays the
same.
...
disprove \(H_0\), and as the alternative is true \(mu>0\) we get small
p-value.
...
Assuming \(mu=1\) results in stochastically lower p-values compared to
when \(mu=0.5\).
...
increased because the mu is farther from \(mu_0 = 0\), and because the
power is an increasing monotone as a function of mu (by definition).
